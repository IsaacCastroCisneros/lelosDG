...
export const AccordionContainer = (props:accordeonPrograma) => 
{
  ...
  return (
    <div className='w-[100%]'>
      <button className="bg-[#978bd6] text-[22px] listaDeProgramas:text-[16px] font-extrabold items-center text-[#fff] py-[16px] px-[48px] rounded-[.5rem] w-[100%] flex justify-between listaDeProgramas:px-[30px] listaDeProgramas1:text-[14px] listaDeProgramas1:px-[15px]"
        onClick={handleAbrirSub}
        >
        <span className="block text-left whitespace-nowrap overflow-hidden text-ellipsis" title={label}>{label}</span>
        <strong className=' leading-[1.5rem]'>{abrirSub ? "+":"-"}</strong>
      </button>
